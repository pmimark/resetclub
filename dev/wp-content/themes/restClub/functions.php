...
add_action( 'init', 'codex_challenge' );

function codex_testi() {
  $labels = array(
    'name' => 'Testimonial',
...
function myplugin_add_meta_box() {
	...
	$screens = array( 'challenge');

	foreach ( $screens as $screen ) {

		add_meta_box(
			'myplugin_sectionid',
			__( 'Manage Challenege', 'myplugin_textdomain' ),
...
add_image_size( 'testimonial', 62, 52, true );
...
<?php
 ...
 $no_days=get_post_meta($post->ID,"no_days",true);
...
<p>
	...
	<input type="text" class="datepicker" name="start_date" value="<?php echo $start_date; ?>" readonly />
</p>
<p>
	...
	{	
	?>
		<option value="<?php echo $i; ?>" <?php if($no_days==$i) { echo 'selected'; } ?>><?php echo $i; ?></option>
	<?php 
	}
	?>
	</select>
...
<p>
	...
	<select name="status"> 
		<option value="0" <?php if($status==0) { echo 'selected'; } ?>>Un-Active</option>
...
function myplugin_add_meta_box1() {
	...
	foreach ( $screens as $screen ) {
		...
		add_meta_box(
			'myplugin_sectionidd',
			__( 'Select Badges', 'myplugin_textdomain' ),
 ...
 * @param WP_Post $post The object for the current post/page.
 */
function myplugin_meta_box_callback1( $post ) {
...
<?php 
global $post;
query_posts('post_type=badge&showpost=-1'); ?>
<?php while (have_posts()) : the_post(); ?>
<li><input type="radio" name="badge" value="<?php echo $post->ID; ?>" <?php if($badge==$post->ID) { echo 'checked'; }  ?>><?php the_title(); ?></li>
...
function myplugin_save_meta_box_data( $post_id ) {
	...
	update_post_meta($post_id,"no_days",$no_days);
	update_post_meta($post_id,"end_date",$end_date);  
	update_post_meta($post_id,"status",$status);  
	update_post_meta($post_id,"badge",$badge);    
	if($status==1)
	{
		foreach( $wpdb->get_results("select * from rest_posts where post_type='challenge' and post_status='publish' and ID!='".$post_id."'") as $key => $row)
		{
			$post_idd = $row->ID;
			update_post_meta($post_idd,"status",0);  
		} 
...
function get_like_dislike($post_id,$user_id)
{
	global $wpdb;	
	foreach( $wpdb->get_results("SELECT count(id) 'total' FROM rest_user_like_dislike where post_id='".$post_id."' and user_id='".$user_id."'") as $key => $row)
	{
		$count = $row->total; 
	}
	if($count==0)
...
add_action( 'init', 'codex_badges' );

function codex_announcement() {
  $labels = array(
    'name' => 'Announcement',
...
add_action( 'init', 'codex_announcement' );